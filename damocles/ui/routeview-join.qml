...
RouteView {
	...
	Connections {
		...
		onMatchmakingQueryCompleted: {
			if ( stepServerList === 1 ) { // if currently searching LAN servers
                ...
                // TODO: Also hack, because we need to wait when all items will append to list view we
                // can't just parse ui items on route change end like we do for rest screens, need to improve that.
    ...
    anchors.topMargin: 128 * Theme.heightScale
    anchors.bottomMargin: 72 * Theme.heightScale
